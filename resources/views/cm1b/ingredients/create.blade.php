...
<main class="content">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <form method="post" action="{{ route('cm1b.ingredients.store') }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
                    @csrf
                    @method('post')

                    <div class="card ">
                        <div class="card-header card-header-primary content-header">
                            <h4 class="card-title">{{ __('Add Ingredients') }}</h4>
                            <a href="{{ route('cm1b.ingredients.index') }}" class="btn btn-sm btn-primary text-white">{{ __('Back to list') }}</a>
                        </div>
                        <div class="card-body ">
                            <div class="container">
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('ing') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('ing') ? ' is-invalid' : '' }}" name="ing" id="input-ing" type="text" placeholder="{{ __('Ingredient Name') }}" value="{{ old('ing') }}" required="true" aria-required="true"/>
                                            @if ($errors->has('ing'))
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('ing_priority') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('ing_priority') ? ' is-invalid' : '' }}" name="ing_priority" id="input-ing_priority" type="number" placeholder="{{ __('1') }}" required />
